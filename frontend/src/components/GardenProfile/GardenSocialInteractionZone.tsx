...
interface GardenSocialInteractionZoneProps {
...
const GardenSocialInteractionZone = ({
  gardenOwnerId,
  initialAppreciations,
  initialComments,
  profileUrl,
  ...
  currentUserId,
  ownerName,
}: GardenSocialInteractionZoneProps) => {
  ...
  const handleAppreciate = async () => {
    if (!isAuthenticated) {
      alert('Please log in to appreciate this garden');
      return;
    }

    try {
      const response = await gardensAPI.appreciate(gardenOwnerId);
  ...
  const handleAddComment = async (e: React.FormEvent) => {
    ...
    try {
      const response = await gardensAPI.addComment(gardenOwnerId, newComment);
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `${ownerName}'s Garden on SproutSync`,
          url: profileUrl,
        });
      } catch (error) {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm rounded-xl shadow-md p-4 sm:p-6">
      ...
      <div className="flex flex-wrap gap-2 sm:gap-4 mb-4 sm:mb-6">
        ...
        >
          <Heart className={`w-4 h-4 sm:w-5 sm:h-5 ${appreciated ? 'fill-current' : ''}`} />
      ...
      {showQR && (
        <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-gray-50 rounded-lg flex flex-col items-center">
          ...
          <p className="mt-2 text-xs sm:text-sm text-gray-600 text-center">Scan to view this garden</p>
        </div>
      )}
...
export default GardenSocialInteractionZone;
