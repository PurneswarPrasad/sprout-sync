...
import fertiliserAnimation from '../../assets/animations/fertiliser.json';
import plantGrowingAnimation from '../../assets/animations/plant.json';

interface FertilizingAnimationProps {
  show: boolean;
  onComplete?: () => void;
}

/**
 * FertilizingAnimation Component
 * 
 * A reusable component that plays two Lottie animations sequentially when a fertilizing task is completed.
 * 
 * Features:
 * - Sequential animation playback (fertiliser → plant growing)
 ...
 */
const FertilizingAnimation: React.FC<FertilizingAnimationProps> = ({ show, onComplete }) => {
  const [isVisible, setIsVisible] = useState(false);
  const fertiliserRef = useRef<any>(null);
  ...
  useEffect(() => {
    if (show) {
      ...
      // Ensure both animations run at 1.5x
      requestAnimationFrame(() => {
        fertiliserRef.current?.setSpeed(1.5);
        growingRef.current?.setSpeed(1.5);
      });

      // Compute total time and finish
      const totalMs = Math.max(
        getDurationMs(fertiliserAnimation as any, 1.5),
        getDurationMs(plantGrowingAnimation as any, 1.5)
      );
      ...
      {isVisible && (
        ...
        >
          ...
          <div className="relative z-10 flex items-center justify-center">
            <AnimatePresence>
              {/* Fertiliser animation - shifted to the LEFT */}
              <motion.div
            key="fertiliser"
          ...
          >
            <div className="w-48 h-48 sm:w-64 sm:h-64 md:w-80 md:h-80">
              <Lottie
                lottieRef={fertiliserRef}
                animationData={fertiliserAnimation}
                loop={false}
                autoplay={true}
...
export default FertilizingAnimation;
