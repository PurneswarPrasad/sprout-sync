...
interface PlantValidationResponse {
  isPlant: boolean;
  confidence: number;
  reason: string;
}
 ...
 */
export async function validatePlantImage(model: any, imagePart: any): Promise<boolean> {
  try {
...
`;
    ...
    const validationResult: PlantValidationResponse = JSON.parse(text);
